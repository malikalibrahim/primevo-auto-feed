...
- Download BigBuy XML's (uit products_list.txt) via FTP
- Parse + filter
...
ENV in GitHub Actions:
  ...
  AVP_SOURCE = price | pvp_bigbuy   (welke adviesprijs we als ondergrens nemen)
"""
...
# ---------------- XML VELDEN ----------------
# Neem ook pvp_bigbuy en iva mee. Images tot en met image10.
IMAGE_FIELDS = [f"image{i}" for i in range(1, 11)]
FIELDS_BASE = [
    "id", "name", "description",
    "price",          # AVP incl. btw (RRP)
    "pvp_bigbuy",     # BigBuy shopprijs incl. btw
    "pvd",            # DP / inkoop excl. btw
    "iva",            # btw % op productniveau
    "ean13", "stock",
    "category", "brand", "date_upd",
    "width", "height", "depth", "weight"
]
FIELDS = FIELDS_BASE[:3] + FIELDS_BASE[3:6] + FIELDS_BASE[6:9] + IMAGE_FIELDS + FIELDS_BASE[9:]

# ---------------- PARAMS (ENV met defaults) ----------------
...
def _has_any_image(r: dict) -> bool:
    return any((r.get(f) or "").strip() for f in IMAGE_FIELDS)
...
def _calc_fields(pvd: float, avp_inc_choice: float, iva_pct: float) -> dict:
    ...
    s_base = max(minP, s1, s2, avp_inc_choice or 0.0)  # AVP als ondergrens
    s = min(maxP, _round_up_to_euro(s_base) - 0.01)    # *.99 afronding
...
def main():
    ...
    # sorteer: hoogste stock, dan laagste AVP (price)
    ...
    with open(csv_preview, "w", newline="", encoding="utf-8") as f:
        ...
        for r in all_rows:
            ...
            iva_pct   = None if r.get("iva") == "" else _pfloat(r.get("iva"))
            avp_choice = pvp_inc if AVP_SOURCE == "pvp_bigbuy" else price_inc

            row.update(_calc_fields(pvd_val, avp_choice, iva_pct))

            # Preview gebruikt image1 als thumbnail (als die leeg is: eerste niet-lege imageX)
            img = (r.get("image1") or "").strip()
            if not img:
                for k in IMAGE_FIELDS:
                    if (r.get(k) or "").strip():
                        img = r.get(k).strip()
                        break
            row["image_preview"] = f'=IMAGE("{img}")' if img else ""
            w.writerow(row)
    ...
    with open(csv_light, "w", newline="", encoding="utf-8") as f:
        ...
        for r in all_rows:
            row = {"Keep?": ""}
            for k in ["id","name","price","pvp_bigbuy","pvd","iva","ean13","stock",*IMAGE_FIELDS,"brand"]:
                row[k] = r.get(k, "")
            pvd_val   = _pfloat(r.get("pvd"))
            price_inc = _pfloat(r.get("price"))
            pvp_inc   = _pfloat(r.get("pvp_bigbuy"))
